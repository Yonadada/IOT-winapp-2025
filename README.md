...
- `TextBox` : 텍스트 입력을 위한 컨트롤
    ...
    - PasswordChar : 들어가는 텍스트를 암호처럼 숨길 때 사용, 보통  ㅁ한자 키 특수문자 동그라미 사용
...
- `RichTextBox` : 포맷팅이 가능한 텍스트 박스 
    -  MS World, 한글워드프로세서 같은 문장 꾸미기 , 페이지 꾸미기 기능이 가능
    - Rtb~ 로 시작
    - ScrollBars : 컨트롤 오른쪽, 하단에 스크롤바 생성
        - Both 보다는 ForceBoth,, ForcedVertical 사용
    - WorldWrap : 컨트롤 길이를 넘어가면 자동으로 줄바꿈 여부 
...
- 대부분 컨트롤 속성
    - Anchor : 현재 컨트롤을 폼의 사이즈가 조정될 때 어디에 고정시키는지 설정
        ...
        - 오른쪽 하단에 있는 컨트롤 : 보통 Rigth, Bottom
    ...
    - Font : 대표적인 폰트 사용할 것 
        - 폼에서 변경하면 하위 컨트롤은 싹다 같이 변경됨


### Visual Studio 개발 팁
- 코드 조각(Code Snippet)을 최대한 활용(코딩 양을 줄임, 오타 방지)

- 보기 > 개체 브라우저에서 필요한 클래스 검색

- 소스코드가 회색으로 연하게 변하는 것 > 되도록이면 사용을 피하라는 뜻 > Alt + Enter 변경권장사항 확인

### C# 문법
- 기본분법
    1. 형변환  - [소스](./day04/Day04Study/SyntaxWinApp01/FrmMain.cs)
        - 큰 사이즈 데이터형에 작은 바이트 데이터형 값을 할당하면 문제 없이 사용가능(묵시적 형변환)

        - 작은 사이즈 데이터형에 큰 바이트 데이터 형을 할당하면 문법적 오류 발생 -> (명시적 형변환)
...
- `StatusStrip` : 상태표시줄 컨트롤. 서브 컨트롤 추가 가능
    - 이름은 거의 변경 안함. 서브 컨트롤 이름을 변경
     - `StatusLabel` : 글자만 표시, Lbl~ 이름시작
     - `ProgressBar` : 진행바 표시, Pro~ 이름시작
...
- 고급 문법 - [소스](./day05/Day05Study/SyntaxWinApp02/FrmMain.cs)
    ...
    5. 제네릭 -> `추후 학습 필요`
    - 파이썬에는 타입지정이 자유로워서 필요가 없다
    - JAVA, C# 등 엄격한 데이터타입 객체지향 언어에서는 반드시 필요!
    ...
    - 제네릭 있는 경우
    - 위 3 개의 메서드를 아래의 메서드 하나로 퉁칠 수 
    있다 => 효율적
    - 대문자 T는 아무거나 사용해도 무방하지만 대문자 한 글자를 선호
    ...
    - UI 프로그램에 `응답없음` 발생
    - 멀티프로세스 : 한꺼번에 여러개의 프로세스를 실행(ex, crome. nave,...)
    - 프로세스에서 여러 일을 한꺼번에 수행하기 위해 분리한 업무단위 
    - 멀티스레드 : 한 프로세스에서 여러 스레드가 동작하는 것
    - 윈앱은 기본적으로 싱글 스레드( UI가 스레드를 제어 )
    - 응답 없음 자주 발생시, 프로그램의 신뢰도가 낮아진다
    - 콘솔에는 실시간 출력, 텍스트 박스, 프로그래스 바 등 컨트롤 표시가 실시간으로 보이지 않음
...
- 사용법 - [소스](./day08/Day08Study/WpfStudyApp05/MainWindow.xaml)
    1. NuGet 패키지 관리 > 
        ...
        - 참고 > https://github.com/MahApps/ > realse 
